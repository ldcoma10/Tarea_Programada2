...
public class TopListaRegalos extends JFrame {
	...
	public TopListaRegalos(final JFrame frame,PoloNorteInc datos) {
		
		List <regalo>listap=new LinkedList<regalo>();
		List <String>lista=new LinkedList<String>();
		
		int x= datos.lista.size();
	    lista top=new lista();
	    ...
	    Term[] arg={AtomTerm.get(datos.lista.get(i).Nombre),IntegerTerm.get(datos.lista.get(i).Edad),AtomTerm.get(datos.lista.get(i).Pais),a,b,c,d};
	     ...
	     String cadena="";
	        ...
	        try {
				...
				{           
					...
					listap=top.insertarTop(lista, listap);
					a=null;
					b=null;
					c=null;}        	
				else {
	    ...
	    for(int i=0;k>i;i++){
			numeros.add(listap.get(i).contador);}
	    Collections.sort(numeros);	    
	    int cont=1;
	    String cadena="TOP 5";
	    while(numeros.size()!=0 && cont<=5 ){
	    	int i=numeros.size()-1;
	    	int m=listap.size();
	    	for (int j=0;j<m;j++){
	    		if (numeros.get(i)==listap.get(j).contador){
	    			if (listap.get(j).nombreRegalo.equals("mediasdegolfista")){listap.remove(j);numeros.remove(i);break;}
	    		cadena+="\n"+cont+". "+listap.get(j).nombreRegalo;
		...
		lblNewLabel_1.setIcon(new ImageIcon("/home/ldcoma10/workspace1/Diego/src/gc.gif"));
